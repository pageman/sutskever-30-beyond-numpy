"""Compact AlexNet-inspired CNN for paper 07."""
...
def synthetic_imagenet_like(config: AlexNetConfig) -> Tuple[np.ndarray, np.ndarray]:
    n = config.image_size
    xs = []
    ys = []
    for label in range(config.num_classes):
        ...
        if label == 0:
            ...
            np.fill_diagonal(np.fliplr(image[2]), 1.0)
        else:
            image[:, n // 4 : 3 * n // 4, n // 4 : 3 * n // 4] = 1.0
            image[0, :, n // 2 - 1 : n // 2 + 1] += 0.5
        xs.append(np.clip(image, 0.0, 1.0))
        ys.append(label)
    return np.stack(xs), np.asarray(ys, dtype=np.int64)


def init_params(config: AlexNetConfig, seed: int = 7) -> ArrayDict:
    rng = seeded_rng(seed)
    scale = 0.12
    return {
        ...
        "conv2_w": rng.normal(scale=scale, size=(config.out_channels, config.hidden_channels, 3, 3)),
        "conv2_b": np.zeros((config.out_channels,), dtype=np.float64),
...
def forward_tinygrad(
    ...
    features = hidden.conv2d(params["conv2_w"], bias=params["conv2_b"], padding=1).relu()
...
def forward_torch(
    ...
    features = F.relu(F.conv2d(hidden, params["conv2_w"], bias=params["conv2_b"], padding=1))
...
def forward_jax(
    ...
    features = jax.lax.conv_general_dilated(hidden, params["conv2_w"], (1, 1), "SAME", dimension_numbers=dims)
    features = jax.nn.relu(features + params["conv2_b"][None, :, None, None])
