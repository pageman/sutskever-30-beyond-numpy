...
from s30bn.paper08_seq2seq_sets import (
    SetSeqConfig,
    forward_jax,
    forward_numpy,
    forward_tinygrad,
...
def test_seq2seq_sets_backends_match_numpy() -> None:
    config = SetSeqConfig()
    params = init_params(config)
    sets, targets = synthetic_set_batch()
    numpy_result = forward_numpy(params, sets, targets)
    tinygrad_loss, tinygrad_logits = forward_tinygrad(params_to_tinygrad(params), sets, targets)
    torch_loss, torch_logits = forward_torch(params_to_torch(params), sets, targets)
    jax_loss, jax_logits = forward_jax(params_to_jax(params), sets, targets)
...
def test_seq2seq_sets_one_step_improves_loss() -> None:
    config = SetSeqConfig()
    params = init_params(config)
    sets, targets = synthetic_set_batch()

    tinygrad_params = params_to_tinygrad(params)
    before_tinygrad, _ = forward_tinygrad(tinygrad_params, sets, targets)
    train_step_tinygrad(tinygrad_params, sets, targets, config.learning_rate)
    after_tinygrad, _ = forward_tinygrad(tinygrad_params, sets, targets)
    assert after_tinygrad.item() <= before_tinygrad.item()

    torch_params = params_to_torch(params)
    before_torch, _ = forward_torch(torch_params, sets, targets)
    train_step_torch(torch_params, sets, targets, config.learning_rate)
    after_torch, _ = forward_torch(torch_params, sets, targets)
    assert float(after_torch.detach()) <= float(before_torch.detach())

    jax_params = params_to_jax(params)
    before_jax, _ = forward_jax(jax_params, sets, targets)
    _, jax_params = train_step_jax(jax_params, sets, targets, config.learning_rate)
    after_jax, _ = forward_jax(jax_params, sets, targets)
