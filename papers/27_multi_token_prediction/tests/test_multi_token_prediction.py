...
def test_multi_token_backends_match_numpy() -> None:
    config = MultiTokenConfig()
    params = init_params(config)
    contexts, targets = synthetic_mtp_batch()
    numpy_result = forward_numpy(params, contexts, targets)
    tinygrad_loss, tinygrad_logits = forward_tinygrad(params_to_tinygrad(params), contexts, targets)
    torch_loss, torch_logits = forward_torch(params_to_torch(params), contexts, targets)
    jax_loss, jax_logits = forward_jax(params_to_jax(params), contexts, targets)
...
def test_multi_token_one_step_improves_loss() -> None:
    config = MultiTokenConfig()
    params = init_params(config)
    contexts, targets = synthetic_mtp_batch()

    tinygrad_params = params_to_tinygrad(params)
    before_tinygrad, _ = forward_tinygrad(tinygrad_params, contexts, targets)
    train_step_tinygrad(tinygrad_params, contexts, targets, config.learning_rate)
    after_tinygrad, _ = forward_tinygrad(tinygrad_params, contexts, targets)
    assert after_tinygrad.item() <= before_tinygrad.item()

    torch_params = params_to_torch(params)
    before_torch, _ = forward_torch(torch_params, contexts, targets)
    train_step_torch(torch_params, contexts, targets, config.learning_rate)
    after_torch, _ = forward_torch(torch_params, contexts, targets)
    assert float(after_torch.detach()) <= float(before_torch.detach())

    jax_params = params_to_jax(params)
    before_jax, _ = forward_jax(jax_params, contexts, targets)
    _, jax_params = train_step_jax(jax_params, contexts, targets, config.learning_rate)
    after_jax, _ = forward_jax(jax_params, contexts, targets)
