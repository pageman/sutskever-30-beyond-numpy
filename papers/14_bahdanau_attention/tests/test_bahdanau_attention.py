...
def test_bahdanau_torch_and_jax_gradients_and_updated_params_match() -> None:
    config = BahdanauConfig()
    params = init_params(config)
    source, decoder_token, target = sample_pair(config)

    torch_params = params_to_torch(params)
    torch_loss, _, _ = forward_torch(torch_params, source, decoder_token, target, config)
    torch_loss.backward()
    torch_grads = mapping_values_to_numpy(torch_params, grad=True)

    jax_params = params_to_jax(params)
    jax_grads = jax.grad(lambda current: forward_jax(current, source, decoder_token, target, config)[0])(jax_params)
    ...
    with torch.no_grad():
        for tensor in torch_params.values():
            ...
            tensor.grad.zero_()
    _, jax_updated = train_step_jax(jax_params, source, decoder_token, target, config)

    assert_parameter_dict_allclose(mapping_values_to_numpy(torch_params), {"jax": jax_updated}, atol=1e-6)

    _, torch_logits, torch_attention = forward_torch(torch_params, source, decoder_token, target, config)
    _, jax_logits, jax_attention = forward_jax(jax_updated, source, decoder_token, target, config)
    assert np.allclose(torch_logits.detach().numpy(), np.asarray(jax_logits), atol=1e-6)
    assert np.allclose(torch_attention.detach().numpy(), np.asarray(jax_attention), atol=1e-6)
