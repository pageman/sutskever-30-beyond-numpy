...
def test_identity_mappings_backends_match() -> None:
...
def test_identity_mappings_one_step_improves_loss() -> None:
    config = IdentityResNetConfig()
    images, labels = synthetic_identity_dataset(config)
    params = init_params(config)
...
def test_zero_residual_parameters_leave_identity_path() -> None:
    config = IdentityResNetConfig()
    images, labels = synthetic_identity_dataset(config)
    params = zero_residual_params(init_params(config))
    torch_loss, torch_logits = forward_torch(params_to_torch(params), images, labels)
    pooled_identity = identity_features_numpy(images).mean(axis=(2, 3))
    expected_logits = pooled_identity @ params["fc_w"].T + params["fc_b"]
    assert torch_loss.item() >= 0.0
    assert np.allclose(torch_logits.detach().numpy(), expected_logits, atol=1e-8)
